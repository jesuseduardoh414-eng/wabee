...
export const DashboardPage = () => {
    ...
    // ── Tratamiento de color cálido original (basado en los tokens del tema) ──
    const shellStyle: React.CSSProperties = {
        borderColor: 'color-mix(in srgb, var(--border-default), white 12%)',
        background: [
            'linear-gradient(135deg, color-mix(in srgb, var(--chart-4), transparent 90%) 0%, color-mix(in srgb, var(--bg-card), white 18%) 45%, color-mix(in srgb, var(--brand-primary), transparent 86%) 100%)',
            'var(--bg-page)',
        ].join(', '),
    ...
    const cardStyle: React.CSSProperties = {
        ...
        boxShadow: '0 18px 40px color-mix(in srgb, var(--text-strong), transparent 95%)',
    };
    const innerTintStyle: React.CSSProperties = {
        borderColor: 'color-mix(in srgb, var(--border-default), white 14%)',
        background: 'color-mix(in srgb, var(--bg-card), white 32%)',
    };
    ...
    return (
        <div className="min-h-screen rounded-[28px] border p-4 sm:p-6 md:p-8" style={shellStyle}>
            <div className="mx-auto w-full max-w-6xl space-y-6">
                {/* ─── Encabezado: saludo + filtro ─── */}
                <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[var(--text-muted)]">
                            {todayLabel.charAt(0).toUpperCase() + todayLabel.slice(1)}
                        </p>
                        <h1 className="mt-1 text-3xl font-bold tracking-tight text-[var(--text-strong)] sm:text-4xl">
                            {greeting}
                        </h1>
                        <p className="mt-1.5 text-[15px] text-[var(--text-body)]">{subtitle}</p>
                    </div>

                    <div className="flex gap-1 rounded-full border p-1" style={innerTintStyle}>
                ...
                {/* ─── Acción primaria ─── */}
                <section className="rounded-3xl border p-6 sm:p-8" style={cardStyle}>
                    <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex-1">
                            ...
                            <div className="mt-3 flex flex-wrap items-end gap-x-3 gap-y-1">
                                <span className="text-6xl font-bold leading-none tracking-tight text-[var(--text-strong)]">
                                    {unassigned}
                                </span>
                                <span className="mb-1 text-lg font-semibold text-[var(--text-body)]">
                            ...
                            <div className="mt-5 flex flex-wrap gap-3">
                                <Link
                                    ...
                                    className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-[var(--text-inverse)] transition-all hover:brightness-95"
                                    style={{ background: 'linear-gradient(135deg, var(--chart-5) 0%, var(--brand-primary) 100%)' }}
                                >
                                    Ir al inbox <ArrowRight size={16} />
                                </Link>
                                <Link
                                    to="/dashboard/wabee/inbox"
                                    className="inline-flex items-center gap-2 rounded-full border px-5 py-2.5 text-sm font-semibold text-[var(--text-strong)] transition-all hover:bg-[color:color-mix(in_srgb,var(--bg-card),white_30%)]"
                                    style={innerTintStyle}
                                >
                                    Asignar a un agente
                                </Link>
                            </div>
                        ...
                        <div className="grid grid-cols-3 gap-6 rounded-2xl border p-5 lg:gap-8" style={innerTintStyle}>
                ...
                <div
                    ...
                    style={
                        allClear
                            ? { borderColor: 'color-mix(in srgb, var(--state-success), transparent 70%)', background: 'color-mix(in srgb, var(--state-success), transparent 90%)', color: 'var(--state-success)' }
                            : { borderColor: 'color-mix(in srgb, var(--state-warning), transparent 70%)', background: 'color-mix(in srgb, var(--state-warning), transparent 90%)', color: 'var(--state-warning)' }
                    }
                >
                ...
                <section className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
                    <article className="rounded-3xl border p-5 sm:p-6" style={cardStyle}>
                        <div className="mb-4 flex items-center justify-between">
                            <div>
                                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Rendimiento</p>
                                <h2 className="mt-1 text-xl font-bold text-[var(--text-strong)]">Conversaciones, leads e ingresos</h2>
                            </div>
                            <span className="rounded-full border px-3 py-1 text-[11px] font-semibold text-[var(--text-body)]" style={innerTintStyle}>
                        ...
                        <div className="h-[260px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={timeSeries || []}>
                                    ...
                                    <Legend verticalAlign="top" height={32} wrapperStyle={{ fontWeight: 600, fontSize: '12px' }} />
                    ...
                    <article className="rounded-3xl border p-5 sm:p-6" style={cardStyle}>
                        <div className="mb-2 flex items-center justify-between">
                            <div>
                                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">IA y handoff</p>
                                <h2 className="mt-1 text-xl font-bold text-[var(--text-strong)]">Eficiencia operativa</h2>
                            </div>
                        ...
                        <div className="mt-2 flex items-end justify-between rounded-2xl border p-4" style={innerTintStyle}>
                            <div>
                                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">Automatización</p>
                                <span className="text-3xl font-bold tracking-tight text-[var(--text-strong)]">{automationRate}</span>
                            </div>
                            <span className="rounded-full bg-[var(--bg-elevated)] px-3 py-1 text-[11px] font-semibold text-[var(--text-strong)]">
                ...
                <section className="grid gap-4 lg:grid-cols-2">
                    <article className="rounded-3xl border p-5 sm:p-6" style={cardStyle}>
                        <div className="mb-4 flex items-center justify-between">
                            <div>
                                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Equipo</p>
                                <h2 className="mt-1 text-xl font-bold text-[var(--text-strong)]">Desempeño del equipo</h2>
                            </div>
                            <span className="rounded-full border px-3 py-1 text-[11px] font-semibold text-[var(--text-body)]" style={innerTintStyle}>
                            ...
                            {(agents || []).slice(0, 4).map((agent) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-semibold text-[var(--text-strong)]">{agent.agentId.split('-')[0]}</p>
                                            <p className="text-xs text-[var(--text-muted)]">{agent.messagesSent} mensajes</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-lg font-bold text-[var(--text-strong)]">{agent.avgResponseTime}m</p>
                                        <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">respuesta</p>
                                    </div>
                                </div>
                            ))}
                            {!agents?.length && (
                                <div className="rounded-2xl border border-dashed px-4 py-8 text-center text-sm text-[var(--text-muted)]" style={innerTintStyle}>
                    ...
                    <article className="rounded-3xl border p-5 sm:p-6" style={cardStyle}>
                        <div className="mb-4 flex items-center justify-between">
                            <div>
                                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Campaña destacada</p>
                                <h2 className="mt-1 text-xl font-bold text-[var(--text-strong)]">{highlightedCampaign?.name || 'Sin campañas activas'}</h2>
                            </div>
                        ...
                        {highlightedCampaign ? (
                            <div className="grid grid-cols-2 gap-3">
                                <div className="rounded-2xl border p-4" style={innerTintStyle}>
...
function Glance({ label, value }: { label: string; value: number }) {
    return (
        <div>
            <p className="text-3xl font-bold leading-none text-[var(--text-strong)]">{value}</p>
            <p className="mt-1.5 text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--text-muted)]">{label}</p>
        </div>
    );
}

function Metric({ label, value, hint, cardStyle }: { label: string; value: string | number; hint: string; cardStyle: React.CSSProperties }) {
    return (
        <article className="rounded-3xl border p-5 sm:p-6" style={cardStyle}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">{label}</p>
            <strong className="mt-2 block text-4xl font-bold leading-none tracking-tight text-[var(--text-strong)]">{value}</strong>
